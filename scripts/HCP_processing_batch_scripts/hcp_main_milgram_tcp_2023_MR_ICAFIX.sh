...
if [ -z "$taskFix_MultiRun_All" ]; then
    echo "Skipping HCP minimal preprocessing computational step 6, part e: fMRI task ICA-FIX module (multi-run; all task-state runs)."
elif [ $taskFix_MultiRun_All = true ]; then
    echo "Running HCP minimal preprocessing computational step 6, part e: fMRI task ICA-FIX module (multi-run; all task-state runs)..."
    ...
    sh ${path_to_opts_script}post_icafix_file_rename_scratch.sh --subj="${subj}" --fixrun="${taskFix_MultiRun_All}"
...
# This version (msmAll_Task_SingleRun) is performed on each task run separately and corresponds to taskFix_SingleRun above. 
